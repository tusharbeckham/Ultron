...
export const providers = {
  ...
  bai: {
    ...
    capabilities: { streaming: true, cancellation: true, retries: true, models: true, usage: true, conversation: true },
    async askDetailed(prompt, opts = {}) { return openAICompatibleAsk({ provider: 'bai', prefix: 'BAI', base: (process.env.BAI_BASE_URL || 'https://api.b.ai/v1').replace(/\/$/, ''), key: required(process.env.BAI_API_KEY, 'BAI_API_KEY'), model: opts.model || process.env.BAI_MODEL || 'qwen3.8-flash', prompt, messages: opts.messages, stream: opts.stream, onToken: opts.onToken, policy: opts }); },
    async ask(prompt, opts) { return (await this.askDetailed(prompt, opts)).text; },
